...
class ProfilController extends Controller
{
    ...
    public function update(Request $request, $id)
    {
        ...
        $request->validate([
            ...
            'password_baru' => 'nullable|min:6|confirmed', // Password baru minimal 6 karakter dan konfirmasi harus cocok
        ], [
        ...
        // Cek apakah user yang sedang login sesuai dengan ID
        if ($user->id !== Auth::id()) {
        ...
        if ($request->filled('password_baru')) {
            if (!$request->filled('password_sekarang')) {
                return back()->withErrors([
                    'password_sekarang' => 'Password sekarang harus diisi jika ingin mengganti password.',
                ]);
            }
            ...
            if (!Hash::check($request->password_sekarang, $user->password)) {
                return back()->withErrors([
                    'password_sekarang' => 'Password sekarang yang Anda masukkan salah.',
                ]);
            }
            ...
            $successMessage = $successMessage
                ? $successMessage . ' dan password berhasil diperbarui.'
